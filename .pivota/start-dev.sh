...
# best-effort seed. Read the Notes in the catalog entry for limitations.
#
...
# === Optional pre-exec snippet (react-next: allowedDevOrigins overlay + migrate) ===
...
# No-op when DATABASE_URL is absent (compose/DinD owns the DB).
